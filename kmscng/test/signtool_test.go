...
func TestSigntoolECDSAP384(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	certFilePath := generateSelfSignedCertTempFile(t, signer, x509.ECDSAWithSHA384)
	defer os.Remove(certFilePath)
	...
	cmd := exec.CommandContext(ctx, signtoolLocation(), "sign", "/v", "/debug",
		"/fd", "sha384", "/f", certFilePath, "/csp", "Google Cloud KMS Provider",
		"/kc", ckv.Name, fileToSign.Name())
	cmd.Env = []string{
		"KMS_ENDPOINT_ADDRESS=" + srv.Addr.String(),
		"KMS_CHANNEL_CREDENTIALS=insecure",
...
func TestSigntoolRSA2048(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func TestSigntoolRSA3072(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
